...
export default function App() {
  ...
  }, [selectedTopics, categories, topics, getTopicsForCategory]);
    ...
    <div className="bg-gray-50 text-gray-900 dark:bg-gray-900 dark:text-gray-50 min-h-screen font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-12 pt-8">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-800 dark:text-gray-100 leading-tight mb-4">Senior Developer Study Plan</h1>
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">Organize your learning path with a clean, intuitive interface.</p>
        </header>
        ...
        <div className="mb-12 flex flex-col sm:flex-row items-center justify-center gap-4">
          <div className="relative w-full sm:w-1/2 max-w-md">
            <input type="text" placeholder="Search by topic or category..." value={searchTerm} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 transition-colors" />
          ...
          <button onClick={handleAddTopic} className="w-full sm:w-auto bg-slate-600 hover:bg-slate-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors transform hover:scale-105 shadow-lg">
            Add New Topic
          </button>
        </div>
        ...
        <div className="mb-12 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 dark:bg-gray-700">
            <div className="bg-slate-500 h-3 rounded-full transition-all duration-500" style={{ width: `${progressPercentage}%` }}></div>
          </div>
        </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4 flex-wrap">
                <button onClick={handleSelectAll} className="bg-slate-500 hover:bg-slate-600 text-white px-4 py-2 rounded-lg">Select All</button>
                <button onClick={handleUnselectAll} className="bg-slate-500 hover:bg-slate-600 text-white px-4 py-2 rounded-lg">Unselect All</button>
                <button onClick={() => handleUpdateStatusForSelected('To Do')} className="bg-slate-500 hover:bg-slate-600 text-white px-4 py-2 rounded-lg">Mark as To Do</button>
                <button onClick={() => handleUpdateStatusForSelected('In Progress')} className="bg-slate-500 hover:bg-slate-600 text-white px-4 py-2 rounded-lg">Mark as In Progress</button>
                <button onClick={() => handleUpdateStatusForSelected('Done')} className="bg-slate-500 hover:bg-slate-600 text-white px-4 py-2 rounded-lg">Mark as Complete</button>
              </div>
              <div className="flex items-center">
                <input type="checkbox" id="compactView" checked={isCompact} onChange={() => setIsCompact(!isCompact)} className="h-5 w-5 rounded border-gray-300 text-slate-600 focus:ring-slate-500 dark:bg-gray-700 dark:border-gray-600 dark:text-slate-400 dark:focus:ring-slate-400 cursor-pointer" />
                  ...
                  <div className={`flex items-center justify-between mb-6 cursor-pointer group ${draggedItem && draggedItem.type === 'CATEGORY' && draggedItem.item === category ? 'opacity-50' : ''} `}>
                    <div className="flex items-center">
                      <input
                        ...
                        className="h-5 w-5 rounded border-gray-300 text-slate-600 focus:ring-slate-500 dark:bg-gray-700 dark:border-gray-600 dark:text-slate-400 dark:focus:ring-slate-400 cursor-pointer mr-4"
                      />
                      <div draggable onDragStart={(e) => handleDragStart(e, category, 'CATEGORY', index)} onClick={() => handleToggleCategoryCollapse(category)} className="flex items-center cursor-grab">
                        ...
                        <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100 group-hover:text-slate-600 dark:group-hover:text-slate-400 transition-colors">{category}</h3>
                      </div>
                    </div>
                      ...
                      {topicsForCategory.map((topic, topicIndex) => (
                        <div
                          ...
                          className={`relative p-6 rounded-lg transition-all duration-300 border border-gray-200 dark:border-gray-700 ${draggedItem
                            ...
                            && draggedItem.item.id === topic.id ? 'opacity-50' : ''} ${selectedTopics.includes(topic.id) ? 'ring-2 ring-slate-400' : ''} ${topic.completed ? 'bg-green-50 dark:bg-green-950 border-green-300 dark:border-green-700' : 'bg-white dark:bg-gray-800'} ${isCompact ? 'p-3' : 'p-6'}`}>
                          <header className="flex justify-between items-start mb-4">
                            <h2 className={`text-xl font-bold text-gray-800 dark:text-gray-100 ${topic.completed ? 'line-through text-gray-500 dark:text-gray-400' : ''} ${isCompact ? 'text-lg' : 'text-xl'}`}>{topic.Topic}</h2>
                          ...
                          <footer className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div className="flex items-center">
                              <input type="checkbox" checked={selectedTopics.includes(topic.id)} onChange={() => handleSelectTopic(topic.id)} className="h-5 w-5 rounded border-gray-300 text-slate-600 focus:ring-slate-500 dark:bg-gray-700 dark:border-gray-600 dark:text-slate-400 dark:focus:ring-slate-400 cursor-pointer" id={`select-${topic.id}`} />
                            ...
                            {!isCompact &&
                              <div className="flex flex-wrap justify-center sm:justify-end gap-3">
                                {topic.link && (
                                  <a href={topic.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-slate-600 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 dark:focus:ring-offset-gray-900 transition-colors">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`Topic: ${topic.Topic}\n\nNotes: ${topic.Notes}\n\nPractice Exercise: ${topic.PracticeExercise}\n\nKnowledge Covered: ${topic.KnowledgeCovered}`); }} className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-900 transition-colors">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); handleEditTopic(topic) }} className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-900 transition-colors">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-6 w-full max-w-2xl transform transition-all max-h-[90vh] overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="modal-title">
            ...
            <div className="space-y-4">
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Category</label><input type="text" placeholder="e.g., ASP.NET Core" value={currentTopic.Category} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentTopic({ ...currentTopic, Category: e.target.value })} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100" /></div>
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Topic</label><input type="text" placeholder="e.g., Middleware Pipeline" value={currentTopic.Topic} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentTopic({ ...currentTopic, Topic: e.target.value })} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100" /></div>
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Priority</label><select title='High, Medium, Low' value={currentTopic.Priority} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCurrentTopic({ ...currentTopic, Priority: e.target.value as 'High' | 'Medium' | 'Low' })} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100" ><option>Low</option><option>Medium</option><option>High</option></select></div>
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Status</label><select title='To Do, In Progress, Done' value={currentTopic.status} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCurrentTopic({ ...currentTopic, status: e.target.value as 'To Do' | 'In Progress' | 'Done' })} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100" ><option>To Do</option><option>In Progress</option><option>Done</option></select></div>
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Notes</label><textarea placeholder="Key concepts, definitions, etc." value={currentTopic.Notes} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCurrentTopic({ ...currentTopic, Notes: e.target.value })} rows={4} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100" ></textarea></div>
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Practice Exercise</label><textarea placeholder="Coding challenge or task..." value={currentTopic.PracticeExercise} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCurrentTopic({ ...currentTopic, PracticeExercise: e.target.value })} rows={4} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100"></textarea></div>
              <div><label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Knowledge Covered</label><textarea placeholder="What have you learned? Key takeaways..." value={currentTopic.KnowledgeCovered} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCurrentTopic({ ...currentTopic, KnowledgeCovered: e.target.value })} rows={5} className="w-full bg-gray-100 dark:bg-gray-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 text-gray-900 dark:text-gray-100" ></textarea></div>
            </div>
            <div className="mt-6 flex justify-end space-x-4">
              ...
              <button onClick={handleSaveTopic} className="bg-slate-600 hover:bg-slate-700 text-white font-semibold py-2 px-4 rounded-md transition-colors">Save</button>
            </div>
          </div>
        </div>
      )}
